            ...
            for (Method m : methods) {
                if (m.getName().startsWith("get") && !m.getName().equalsIgnoreCase("getClass")) {
                    ...
                    try {
                        ...
                        if (packageName != null) {
                            if (!packageName.getName().contains("co.za.payu")) {
                                returnObject = m.invoke(responseType, (Object[]) null);
                                if (returnObject != null && List.class.isAssignableFrom(m.getReturnType())) {
                                    List listObj = (List) returnObject;
                                    int i = 0;
                                    for (Object o : listObj) {
                                        if (o.getClass().getPackage().getName().contains("co.za.payu")) {
                                            responseMap.putAll(generateMapFromResponse(o, propertyName
                                                                    + "(" + i
                                                                    + ")"));
                                        } else {
                            ...
                            responseMap.put(propertyName, m.invoke(klazz.newInstance(), (Object[]) null));
                        }
